...
import { PitchbendConfig } from '@shared/hardware-config/input-config';
...
describe('fromDriver', () => {
  test('sets values correctly', () => {
    const conf = PitchbendConfig.fromDriver(DRIVER);
...
describe('toJSON', () => {
  test('de/serializes correctly', () => {
    ...
    const result = parse<PitchbendConfig>(json);
...
describe('id', () => {
  test('returns correct id', () => {
    const conf = PitchbendConfig.fromDriver(DRIVER);
    expect(conf.id).toBe('pitchbend.11');
  });
});
